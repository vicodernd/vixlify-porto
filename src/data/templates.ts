...
export const templates: Template[] = [
  {
    slug: 'sonda',
    title: 'SONDA — Deep Water Expeditions',
    category: 'interactive',
    tags: ['GSAP', 'ScrollTrigger', 'SVG', 'Canvas', 'Interactive Map'],
    description:
      'A liveaboard dive expedition site built on one interactive hydrographic chart. Nine stations plotted on real bathymetry, a route that draws itself on scroll, and a live depth profile that cross-sections each site as you hover it. Then a pinned descent from the sunlit layer to midnight, and a sailing year where the swell curve and a hatched monsoon window explain why there are only four departures. Every photograph carries a scroll parallax. Abyssal blue with one bioluminescent accent.',
    prompt: `You are a principal designer and creative developer at a high-end studio, building an Awwwards-grade INTERACTIVE EXPEDITION SITE for a fictional liveaboard dive operator called "SONDA" (from the sounding line, the rope used to measure depth). They run twelve day expeditions to seamounts and outer atolls in the eastern Banda Sea. Build it as a React + Tailwind app. Use framer-motion for reveals and micro-motion, GSAP + ScrollTrigger for the scroll-scrubbed chart and the pinned descent, and Lenis for smooth scroll. No WebGL anywhere: this design is deliberately SVG and canvas 2D, which is part of its character. Quality bar is Awwwards, never a template.

INTERPRETATION NOTE: the live result generated from this prompt is a React + Tailwind interpretation of the demo, not a pixel-exact copy. Keep the full quality; do not simplify the chart, the depth profiles, or the motion to make it easier to reproduce.

THE CONTROLLING IDEA (anti-slop spine)
The chart is the product. Every element on the page is drawn as if it were plotted onto one hydrographic survey chart, so the whole site reads as a working instrument rather than a brochure. Nothing is decoration: the numbers are soundings, the labels are stations, the lines are tracks. Wherever the copy makes a claim, the design has to PROVE it with data rather than assert it. The single accent colour is bioluminescence, and it appears literally in the photography, so the interface and the imagery share one light.

ART DIRECTION
Cold, deep, quiet, serious. This operator is not selling a holiday, it is selling competence at depth. Near-black abyssal blue ground throughout, generous negative space, and a heavy monospace readout layer carrying coordinates, depths, bearings, current and visibility. Photography is real underwater work: open blue water with a single light shaft, a coral wall dropping into black, a school of pelagics, bioluminescent plankton, a deep sea creature under a submersible lamp, a liveaboard at anchor at dawn. Corner-anchored navigation so full-bleed imagery is never interrupted by a bar.

PALETTE (CSS custom properties / Tailwind theme tokens)
--abyss #04121B (ground) · --deep #0A2230 (surface, panels) · --shelf #14384A (recessed, chart water)
--foam #E6EFF0 (ink, a cool off-white, never pure white) · --haze #7E9AA6 (secondary) · --faint #46626E (tertiary, lines)
--lume #7CFFB2 (the ONE bioluminescent accent) · --lume-deep #3ECF86
--line rgba(230,239,240,.12)
Discipline on the accent: --lume is only ever the drawn route, the selected station or departure, a depth figure, one primary call to action, and the live status dot. It is never a large fill.

TYPE
Display: Cabinet Grotesk (Fontshare) 300, sentence case, quiet register at manifesto scale, tracking about -.035em. Hero up to clamp(2.5rem, 6.4vw, 6.4rem). This brand is confident, not loud, so never set the display in shouting uppercase.
Body: Synonym (Fontshare) 400/500, line-height 1.6.
Readout: Azeret Mono 400, uppercase, .62rem, letter-spacing .2em. Use it heavily. A hydrographic chart lives on its labels.
Scale the whole layout proportionally on very wide screens: root font-size 1.111vw above 1440px with everything in rem and no cap, so the composition holds identically at 4K. Never use CSS zoom.

SECTIONS (in order)
1. SOUNDING (loader, once, about 1.2s): abyss ground, three expanding sonar ping rings, and a tabular mono counter running 0000 up to 1840 M with a thin --lume progress line. It lifts, then sets a ready flag that gates every reveal on the page. Gate initialisation on document.fonts.ready, not window load.
2. HERO: a full-bleed looping video of a slow descent along a coral wall, with a still of open blue water as its poster. A faint chart graticule is masked over the water. Corner readouts in mono: vessel position, sea floor depth, heading, water temperature, plus a live ticking UTC clock. One quiet Cabinet Grotesk sentence as the headline, a short lede, and three display-scale stats (max sounding, charted sites, berths aboard).
3. THE CHART (the centrepiece, and where most of the engineering goes): one SVG chart on a 1200 by 700 viewBox. Draw irregular island and reef blobs with concentric bathymetric contour rings around each feature, and scatter about thirty small mono depth soundings across the water whose values get deeper the further they sit from the nearest reef. Plot nine dive stations at irregular positions so the track weaves like a real passage, never like a line graph. Connect them with a Catmull-Rom smoothed path. The route DRAWS ITSELF on scroll scrub via stroke-dasharray and stroke-dashoffset, with the undrawn remainder visible underneath as a dashed planned track, and each station fading in as the line reaches it. Hovering or focusing a station pulls its DEPTH PROFILE into a side panel: an SVG cross-section of that site's actual seabed shape with the dive line traced over it, a marker at working depth, plus max depth, current, visibility and water temperature. Every station has a genuinely different profile: a gentle slope, a shelf that falls off a cliff, a pinnacle rising off the floor, a narrow vertical shaft. Clicking a station opens a full dive brief over the chart with a real photograph, the site notes and what lives on the wall. Stations are real buttons: keyboard focusable, Enter to open, arrow keys to step between them, Escape to close.
4. THE DESCENT: a pinned section about 420vh tall staging a scroll-driven descent through the water column. Three zones get one beat each (Epipelagic 0 to 200 m, Mesopelagic 200 to 1000 m, Bathypelagic below 1000 m), each with its own full-bleed photograph crossfading into the next. The background sinks toward black as you go, a mono depth gauge on the right edge counts up non-linearly to 1840, and the zone name updates with it. Behind everything, a canvas 2D marine snow layer drifts UPWARD, because the camera is sinking, and thins out with depth. Run the canvas loop only while the section is on screen.
5. THE VESSEL: a graded aerial photograph of the boat at anchor beside a mono specification sheet as bordered rows (length overall, guest berths, crew, range, compressors, recompression chamber, tenders, watermaker). No feature grid, no icons. Copy should imply hard-won experience rather than list amenities.
6. THE SAILING YEAR (the second interactive set-piece, and the anti-slop moment of the lower page): the headline says there are only four departures a year and that the weather decides, so DRAW THE WEATHER. An SVG axis of twelve months carries a filled mean-swell curve (significant wave height in metres, peaking hard in the monsoon and bottoming out mid-year). Hatch the closed months across the full height and label them CLOSED · MONSOON. Then place the four departures as small selectable bars sitting in the calm trough, each on a thin stem down from the swell curve at its month, each carrying its index and its remaining-berth count. Selecting one (click, Enter, or arrow keys between bars) redraws, in a panel below, THAT DEPARTURE'S OWN LEG: the same reef shapes and the same 1200 by 700 chart coordinates as section 3, the full route ghosted in a faint dashed line, and only that trip's stations joined by a bright --lume track with their names labelled. Beside it, a bordered row list where the deepest-dive figure is COMPUTED from the stations actually in that leg, plus nights aboard, berths left as lit dots, and price. Do NOT build this section as a hover list that pops a floating image near the cursor; that device is overused and it proves nothing.
7. REQUEST A BERTH: an editorial photograph of a diver checking gear on the dive deck beside a short form with underline-only fields (name, email, departure, logged dives) and a pill submit that inverts to --lume on hover. Close with a mono credentials row. The copy should be honest about turning applicants away.
8. FOOTER: chart styling. Link columns, a live status bar (coordinates, sea floor, next departure, UTC clock), a --lume sounding line that draws downward on scroll, and a giant outlined Cabinet Grotesk SONDA wordmark clipped by the bottom edge that rises as you reach it.

MOTION
One principle: everything sinks. Reveals enter from above and settle downward, never rise up, because the whole page is a descent. Transitions use cubic-bezier(.4,0,.2,1) at 0.4 to 0.6s, and every scrubbed timeline carries scrub: 0.6 for smoothing. Motion stays on at all times; do not gate it behind a reduced motion query.

Every photograph on the page carries a scroll PARALLAX, so passing one reads as depth rather than as a slide. Over-scale the image inside an overflow-hidden frame, then drift it vertically on scrub as the frame crosses the viewport: about scale 1.2 with a travel of plus or minus 7 percent for the vessel and dive-deck photographs, and in the pinned descent let the three water-column images both drift downward and creep closer as the depth gauge climbs. The hero video gets the same treatment at a gentler rate. THE RULE THAT MATTERS: the drift must never exceed the scale overhang, which is (scale minus 1) divided by 2, or the frame opens up along one edge. If the scale is animated at the same time, satisfy that inequality across the whole progress range, not just at the end. Release will-change on the scroll trigger's leave callbacks rather than leaving every image on its own compositor layer.

IMPLEMENTATION NOTES THAT MATTER
Never use .from() inside a scrubbed GSAP timeline: set the start state explicitly with gsap.set() and then .to() it, otherwise elements strand at opacity 0 when the user lands mid-page. Do not centre an element with transform: translateY(-50%) if GSAP will also animate y on it, because GSAP rewrites the whole transform and silently deletes the centring, leaving the block hanging off the bottom edge; centre it with top:0, bottom:0 and align-content: center instead, and give the pinned copy asymmetric vertical padding so it clears the header above and the status readouts below. In a scrubbed timeline, the positions you write are only scroll fractions if the timeline's total duration is exactly 1, so pad it (a zero-effect tween of duration 1 at position 0) before assuming that 0.7 means seventy percent of the scroll; otherwise the last beat finishes right as the section unpins and nobody ever reads it. Because the descent is pinned at 100svh, test the tallest zone at short viewport heights, not just at 1080. The navigation is fixed, so hide it on scroll down, return it on scroll up, and only give it a frosted scrim once it is past the hero. Put the floating status readouts on their own bottom gradient scrim so they never fight the content passing under them, and stand them down when the footer's own status bar arrives. In the departure bars, scope the selected-state text colour to the index label only, or it will also repaint the berth count underneath the bar and make it invisible against the ground. Watch class-name collisions between a small legend swatch and a large layout container that share a word like "dep". On tablet and mobile both the chart and the year axis carry too much information to shrink, so let each scroll horizontally inside its own frame at a comfortable minimum width with a drag hint, and remember to set min-width: 0 on the grid and flex ancestors or the SVG will blow out the page width.

QUALITY BAR
Award-winning, not a template. Real editorial rhythm, deliberate type scale contrast, one coherent palette applied with restraint, generous whitespace, copy specific to expedition diving rather than filler. No emoji rows, no three identical feature cards, no gradient blob hero. It must be fully responsive and must hold its composition at 4K.

Note: the live result generated from this prompt is a React + Tailwind interpretation of the demo, not a pixel-exact copy.`,
    thumb: '/templates/sonda/thumb.webp',
    demoUrl: '/templates/sonda/index.html',
    date: '2026-08-08',
  },
  {
    slug: 'seve',
